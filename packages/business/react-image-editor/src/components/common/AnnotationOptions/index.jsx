...
import { StyledOptionPopupContent, StyledOptions, StyledIconWrapper } from './AnnotationOptions.styled';
...
const AnnotationOptions = ({
  ...
  return (
    <StyledOptions className={`FIE_annotations-options${className ? ` ${className}` : ''}`}>
      {!hideFillOption && <ColorInput color={annotation.fill} onChange={changeAnnotationFill} />}
      {children}
      {options.map(
        option =>
          option && (
      ...
      {OptionPopupComponent && (
        <Menu
          ...
          <StyledOptionPopupContent>
            <OptionPopupComponent annotation={annotation} updateAnnotation={updateAnnotation} {...rest} />
          </StyledOptionPopupContent>
        </Menu>
      )}
    </StyledOptions>
